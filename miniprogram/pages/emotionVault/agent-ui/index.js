...
Component({
  ...
  methods: {
    ...
    sendMessage: async function (event) {
      ...
      // 检测是否是角色提示词，如果是则不显示为用户消息

      const chatMode = this.data.chatMode;
      // console.log(inputValue,bot.botId)

      // 检测是否是角色提示词
      const isRolePrompt = inputValue.includes('你现在扮演的角色是') &&
                          (inputValue.includes('请严格按照以上设定进行对话') ||
                           inputValue.includes('保持角色特征的一致性'));

      // 如果是角色提示词，则不显示为用户消息
      let newChatRecords = [...chatRecords];

      // 如果是角色提示词，则将其作为系统提示词处理
      if (isRolePrompt) {
        console.log('检测到角色提示词，不显示为用户消息');
        // 如果有角色信息，则将提示词保存到角色信息中
        if (agentConfig.roleInfo) {
          agentConfig.prompt = inputValue;
        }
      } else {
        const userRecord = {
          content: inputValue,
        ...
        newChatRecords.push(userRecord);
      }
      ...
      if (chatMode === "bot") {
        ...
        // 将角色提示词作为第一条消息添加到历史记录中
        if (agentConfig.roleInfo) {
          // 优先使用预设的prompt
          const systemPrompt = agentConfig.prompt || generateSystemPrompt(agentConfig.roleInfo);
          if (systemPrompt) {
            history.push({
              role: "user",  // 作为用户消息发送角色设定
              content: systemPrompt
            });
            console.log('发送的角色提示词:', systemPrompt);
          }
        }
