...
- **提示词优化与功能扩展**
  - 优化了`geminiModel.js`中的提示词设计，参考`bigmodel.js`中的提示词格式
  - 增强了情感分析功能，支持更详细的情感维度分析
  - 添加了关键词聚类、用户兴趣分析和报告生成等功能
  - 实现了与智谱AI模块相同的接口和返回格式，确保兼容性
  - 优化了JSON解析逻辑，提高了解析成功率

- **历史消息处理优化**
  - 添加了对历史消息的处理支持，提高对话连贯性
  - 优化了消息格式转换，适配Gemini API的请求格式
  - 实现了系统提示词的转换，解决Gemini不支持system角色的问题
...
### 新增：多模型支持
HeartChat 现已支持多种AI模型，包括智谱AI、Google Gemini、OpenAI、Crond API和CloseAI，旨在为用户提供更多样化、更高质量的AI交互选择。

#### 多模型集成
通过模块化设计和统一接口，应用可以无缝切换不同的AI模型平台和具体模型，支持动态获取模型列表。

详细的架构设计、配置方法和使用指南，请参考：
- **[多模型集成文档](docs/使用文档/多模型集成文档.md)**

#### 支持的模型平台
- **智谱AI**：glm-4-flash, glm-4
- **Google Gemini**：gemini-2.5-flash-preview-04-17
- **OpenAI**：gpt-3.5-turbo, gpt-4, gpt-4-turbo
- **Crond API**：gpt-4o-mini, deepseek-v3, o3-mini
- **CloseAI**：deepseek-ai/DeepSeek-V3-0324

#### 模型选择功能
在聊天界面，用户可以实时切换不同的AI模型平台和具体模型，体验不同模型的特点和能力。模型选择器组件提供了两级选择界面：
1. 第一级：选择模型平台（智谱AI、Google Gemini、OpenAI、Crond API、CloseAI）
2. 第二级：选择具体模型（如 gpt-4o-mini, deepseek-v3, o3-mini 等）

这些集成为用户提供了更多样化、更高质量的AI交互选择，并为未来实现更高级的AI功能（如多模态交互、复杂内容创作）奠定基础。
